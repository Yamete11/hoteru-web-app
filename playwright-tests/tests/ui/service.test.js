...
test('Edit a service', async ({ page }) => {
    ...
    await serviceDetailsPage.fillForm(
...
test.afterAll(async ({ browser }) => {
    ...
    const tryDelete = async (title) => {
        try {
            await servicePage.enterSearch(title);
            await page.waitForTimeout(400);
            await servicePage.deleteServiceByTitle(title);
        } catch (_) {}
    };

    await tryDelete(testData.editServiceTitle);
    await tryDelete(testData.serviceTitle);
